...
public class BackgroundEffect {
	...
	public void init(){
        ...
        /**
         * TODO: Render stars onto a texture and use the parallax matrix to give illusion
         */
...
/*		final int STAR_DENSITY = 44;
		...
		for(int i = 0; i < stars.size(); i++){
			...
			for(int j = 0; j < STAR_DENSITY; j++){
				...
				float x = MathUtils.random(camera.getModifiedPosition().x, camera.getModifiedPosition().x + camera.getOrtho().viewportWidth);
				float y = MathUtils.random(camera.getModifiedPosition().x, camera.getModifiedPosition().x + camera.getOrtho().viewportHeight);
	...
	public void render(float delta, SpriteBatch sb){
		...
		float width = camera.getOrtho().viewportWidth;
		float height = camera.getOrtho().viewportHeight;
